...
bool full_duplicate_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    for (int i = 0 ; i < size ; i++)
    {
        double_input[i] = ((double) pow(-1, i))*sqrt(i+1);
    }
    ...
    for (int i = 0 ; i < size ; i++)
    {
        complex_input[i] = (((double) pow(-1, i))*(i+1)) + (((double) pow(-1, i+1))*1/(i+1))*I;
    }
    ...
    // full_duplicate the int matrix
    Matrix *mat = int_matrix;
    function_comparison_M("full_duplicate", full_duplicateM_ref, full_duplicateM_vec, mat, notes, accuracy_check, &success);

    // full_duplicate the double matrix
    mat = double_matrix;
    function_comparison_M("full_duplicate", full_duplicateM_ref, full_duplicateM_vec, mat, notes, accuracy_check, &success);

    // full_duplicate the complex matrix
    mat = complex_matrix;
    function_comparison_M("full_duplicate", full_duplicateM_ref, full_duplicateM_vec, mat, notes, accuracy_check, &success);
...
{
    char *comment="Normal";
    return(run_test(argc, argv, full_duplicate_vec_test,"full_duplicateM", SINGLE_MATRIX, comment));
}