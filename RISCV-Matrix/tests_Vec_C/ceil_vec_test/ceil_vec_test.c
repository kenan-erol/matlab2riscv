...
bool ceil_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    function_comparison_M("ceil", ceilM_ref, ceilM_vec, mat, notes, accuracy_check, &success);
    
    // ceil the double matrix
    mat = double_matrix;
    function_comparison_M("ceil", ceilM_ref, ceilM_vec, mat, notes, accuracy_check, &success);

    // ceil the complex matrix
    mat = complex_matrix;
    function_comparison_M("ceil", ceilM_ref, ceilM_vec, mat, notes, accuracy_check, &success);
...
{
    char *comment="Normal";
    return(run_test(argc, argv, ceil_vec_test,"ceilM", SINGLE_MATRIX, comment));
}