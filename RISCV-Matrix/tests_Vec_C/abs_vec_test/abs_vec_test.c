...
bool abs_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    // Absolute value the int matrix
    Matrix *mat = int_matrix;
    function_comparison_M("abs", absM_ref, absM_vec, mat, notes, accuracy_check, &success);

    // Absolute value the double matrix
    mat = double_matrix;
    function_comparison_M("abs", absM_ref, absM_vec, mat, notes, accuracy_check, &success);

    // Absolute value the complex matrix
    mat = complex_matrix;
    function_comparison_M("abs", absM_ref, absM_vec, mat, notes, accuracy_check, &success);
...
{
    char *comment="Normal";
    return(run_test(argc, argv, abs_vec_test,"absM", SINGLE_MATRIX, comment));
}