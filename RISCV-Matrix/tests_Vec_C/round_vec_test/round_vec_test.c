...
bool round_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    for (int i = 0 ; i < size ; i++)
    {
        double_input[i] = ((double) pow(-1, i))*(i+1)/4;
    }
    ...
    for (int i = 0 ; i < size ; i++)
    {
        complex_input[i] = (((double) pow(-1, i))*(i+1)/4) + (((double) pow(-1, i+1))*(i+1)/4)*I;
    }
    ...
    // round the int matrix
    Matrix *mat = int_matrix;
    function_comparison_M("round", roundM_ref, roundM_vec, mat, notes, accuracy_check, &success);

    // round the double matrix
    mat = double_matrix;
    function_comparison_M("round", roundM_ref, roundM_vec, mat, notes, accuracy_check, &success);

    // round the complex matrix
    mat = complex_matrix;
    function_comparison_M("round", roundM_ref, roundM_vec, mat, notes, accuracy_check, &success);
...
int main(int argc, char *argv[]) 
{
    // return(false);
    char *comment="Normal";
    return(run_test(argc, argv, round_vec_test,"roundM", SINGLE_MATRIX, comment));
}