...
bool arg_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    // arg value the int matrix
    Matrix *mat = int_matrix;
    function_comparison_M("arg", argM_ref, argM_vec, mat, notes, accuracy_check, &success);

    // arg value the double matrix
    mat = double_matrix;
    function_comparison_M("arg", argM_ref, argM_vec, mat, notes, accuracy_check, &success);

    // arg value the complex matrix
    mat = complex_matrix;
    function_comparison_M("arg", argM_ref, argM_vec, mat, notes, accuracy_check, &success);
...
{
    char *comment="Normal";
    return(run_test(argc, argv, arg_vec_test,"argM", SINGLE_MATRIX, comment));
}