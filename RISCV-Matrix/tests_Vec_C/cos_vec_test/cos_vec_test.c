...
bool cos_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    for (int i = 0 ; i < size ; i++)
    {
        double_input[i] = ((double) pow(-1, i))*i*M_PI/6;
    }
    ...
    for (int i = 0 ; i < size ; i++)
    {
        complex_input[i] = (((double) pow(-1, i))*(i+1)/7) + (((double) pow(-1, i+1))*1/(i+1)/7)*I;
    }
    ...
    // cosolute value the int matrix
    Matrix *mat = int_matrix;
    // function_comparison_M("cos", cosM_ref, cosM_vec, mat, notes, accuracy_check, &success);

    // cosolute value the double matrix
    mat = double_matrix;
    function_comparison_M("cos", cosM_ref, cosM_vec, mat, notes, accuracy_check, &success);

    // cosolute value the complex matrix
    mat = complex_matrix;
    // function_comparison_M("cos", cosM_ref, cosM_vec, mat, notes, accuracy_check, &success);
...
int main(int argc, char *argv[]) 
{
    return(false);
    // char *comment="Normal";
    // return(run_test(argc, argv, cos_vec_test,"cosM", SINGLE_MATRIX, comment));
}