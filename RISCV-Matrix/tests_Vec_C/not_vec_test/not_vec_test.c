...
bool not_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    for (int i = 0 ; i < size ; i++)
    {
        double_input[i] = ((i+1))/150000000000000;
    }
    ...
    function_comparison_M("not", notM_ref, notM_vec, mat, notes, accuracy_check, &success);

    // NOT the double matrix
    mat = double_matrix;
    function_comparison_M("not", notM_ref, notM_vec, mat, notes, accuracy_check, &success);

    // NOT the complex matrix
    mat = complex_matrix;
    function_comparison_M("not", notM_ref, notM_vec, mat, notes, accuracy_check, &success);
...
{
    char *comment="Normal";
    return(run_test(argc, argv, not_vec_test,"notM", SINGLE_MATRIX, comment));
}
