...
bool or_vec_test(int s1, int s2, bool accuracy_check)
{
    bool success = true;
    ...
    function_comparison_MM("or", orM_ref, orM_vec, mat, int_mask, notes, accuracy_check, &success);
    function_comparison_MM("or", orM_ref, orM_vec, mat, double_mask, notes, accuracy_check, &success);
    function_comparison_MM("or", orM_ref, orM_vec, mat, complex_mask, notes, accuracy_check, &success);
...
{
    char *comment="Normal";
    return(run_test(argc, argv, or_vec_test,"orM", MATRIX_MATRIX, comment));
}
