...
The successful chronic implantation of next-gen BCIs is constrained by extreme power efficiency requirements due to patient safety concerns. This has historically been met through highly inflexible, monolithic ASIC hardware. HALO BCIs diverge from the norm by using various function-specific PEs and the RVV extension to introduce significant flexibility without compromising critical thermal limits. However, a highly optimized software compilation pipeline is required to make HALO and its clinical flexibility and capabilities accessible to the broader neuroscientific community. This project directly addresses this need by engineering an end-to-end toolchain that pairs a TypeScript-based MATLAB-to-C compiler with a RVV-optimized matrix library, enabling the automatic transformation of high-level MATLAB algorithms into power-optimized RISC-V binaries.
...
The vectorized library runs correctly on Spike and is faster than the scalar reference for most kernels. For the kernels for which it is not faster, and for the kernels that regressed in their vec-ref ratios, the most likely culprit is compiler toolchain updates across three years of development. New functions have been vectorized, increasing the parallel capabilities of the matrix library. These updates have yielded improvements of up to 95.43\% for the \texttt{mean} test case on integer types. In addition to the vectorization efforts, the semantic translation layer has been benchmarked and improved for accuracy, bringing the number of passing tests up from 68 to 84 out of 88 tests.
...
The hand-off to the BCI lab at the Yale School of Medicine will enable physical HALO testing under neuroscientist-written full workloads, where the 15~mW power envelope and $1\,^{\circ}$C thermal limit will provide a real-world check on the cycle-count conclusions drawn here. Actual power consumption, memory bandwidth saturation, and other factors---none of which are modeled by \texttt{rdinstret}-based benchmarking on Spike---will then become the dominant performance factors to measure and analyze.
